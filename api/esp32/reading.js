...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    return res.status(200).json({ success: true, message: 'Float reading persisted', received: reading });
  } catch (err) {
    console.error('Supabase insert error', err);
    return res.status(502).json({ success: false, error: 'Failed to persist reading' });
  }
};
